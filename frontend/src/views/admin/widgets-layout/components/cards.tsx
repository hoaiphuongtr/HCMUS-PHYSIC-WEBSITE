...
export const NewsOverlayCard: ComponentConfig<{
  ...
  render: ({ imageUrl, title, excerpt, date, linkUrl, size, puck }) => {
    const heights: Record<string, string> = { sm: "h-48", md: "h-64", lg: "h-80" };
    return (
      <a
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          {date && <span className="text-xs text-white/60 uppercase tracking-wider">{date}</span>}
          <h3 className="text-white font-bold text-lg mt-1 line-clamp-2 group-hover:underline">{title}</h3>
          {excerpt && <p className="text-white/70 text-sm mt-1 line-clamp-2">{excerpt}</p>}
        </div>
      </a>
    );
  },
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span className="text-xs uppercase">{months[parseInt(month)] || month}</span>
        </div>
        <div className="min-w-0">
          <h4 className="font-semibold text-slate-900 group-hover:text-blue-700 transition-colors line-clamp-2">{title}</h4>
