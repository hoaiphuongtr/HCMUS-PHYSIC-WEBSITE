...
export function MediaPickerModal({ onSelect, onClose }: MediaPickerModalProps) {
  ...
  return (
    <ModalPortal>
      ...
      >
        <div
          className="bg-white dark:bg-[#1a2436] rounded-xl shadow-2xl max-w-4xl w-full max-h-[85vh] flex flex-col"
        ...
        >
          <header className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4">
            ...
            <input
              ...
              placeholder="Tìm…"
              className="w-56 px-3 py-1.5 text-xs border border-slate-200 dark:border-slate-800 rounded-lg outline-none focus:ring-2 focus:ring-blue-200"
            />
            <button
              type="button"
              onClick={onClose}
          ...
          {(tagsQuery.data?.length ?? 0) > 0 && (
            <div className="px-5 py-2 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-1">
              <button
                type="button"
                onClick={() => {
          ...
          {totalPages > 1 && (
            <footer className="px-5 py-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-center gap-2">
              <button
                type="button"
                disabled={page === 1}
                onClick={() => setPage(page - 1)}
                className="px-2.5 py-1 text-xs font-medium text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-800 rounded-lg disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-[#202c44]"
              >
                Trước
              </button>
