...
| Quy trình xuất bản | Trạng thái nháp → chờ xuất bản (tự động khi gắn vào bố cục) → xuất bản; lên lịch xuất bản tự động; lịch sử phiên bản bố cục và khôi phục |
...
Lược đồ đầy đủ được khai báo tập trung trong tệp lược đồ Prisma; Hình 3.3 trình bày rút gọn các thực thể trung tâm và quan hệ giữa chúng. Lược đồ dữ liệu đầy đủ với 33 thực thể và 43 quan hệ khóa ngoại được trình bày tại Phụ lục A.
...
Bài viết tuân theo một máy trạng thái phản ánh vòng đời nội dung. Trạng thái khởi đầu là nháp (DRAFT): bài đang được soạn nội dung và chưa gắn vào bố cục nào, nên chưa hiển thị công khai. Khi bài được gắn vào một bố cục (mục 3.5.2), hệ thống tự chuyển bài sang chờ xuất bản (PENDING), đánh dấu nội dung đã có trang trình bày và sẵn sàng đưa lên. Từ đó biên tập viên chọn xuất bản ngay (PUBLISHED) hoặc lên lịch (SCHEDULED, được tác vụ định kỳ tự chuyển sang đã xuất bản khi đến hạn). Việc hiển thị công khai của một bài viết gắn với việc bố cục chứa nó được xuất bản, chứ không riêng với trạng thái của bản thân bài (mục 3.5.2). Sơ đồ chuyển trạng thái được minh họa trong Hình 3.5.

【HÌNH 3.5, Sơ đồ trạng thái bài viết: DRAFT →(gắn bố cục) PENDING → PUBLISHED; PENDING → SCHEDULED → PUBLISHED (cron)】

*Hình 3.5. Máy trạng thái của bài viết*

### 3.5.2 Sao chép mẫu giao diện và chèn dữ liệu bài viết

Điểm đặc thù nhất của quy trình xuất bản là cách một *bài viết dạng dữ liệu* trở thành một *trang hoàn chỉnh*. Bài viết trong hệ thống là dữ liệu có cấu trúc (tiêu đề, thân bài, ảnh bìa, thẻ…), không phải HTML; để hiển thị, nó được "rót" vào một bố cục mẫu. Khi biên tập viên chọn xuất bản bài viết theo mẫu, một điểm cuối API chuyên trách thực hiện bốn bước: đọc bố cục mẫu chứa các khối giữ chỗ; sao chép sâu cây bố cục của mẫu; duyệt đệ quy toàn bộ cây (kể cả các vùng thả lồng nhau) và thay thuộc tính của sáu loại khối giữ chỗ (tiêu đề, thân bài, ảnh bìa, danh sách thẻ, thông tin sự kiện và đầu trang bài viết) bằng dữ liệu thật của bài viết; cuối cùng lưu kết quả thành một bố cục nháp mới kèm liên kết trỏ về bài viết gốc, đồng thời nếu bài đang ở trạng thái nháp thì tự chuyển sang chờ xuất bản. Thuật toán duyệt – chèn được hiện thực dưới dạng một hàm thuần túy, và mỗi khối sau khi chèn được đánh dấu để lần đồng bộ sau nhận diện. Toàn bộ luồng được minh họa trong Hình 3.6.
